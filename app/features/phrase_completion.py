...
class PhraseCompleter:
    ...
    def _load_musteri_phrases(self):
        """musteri_hizmetleri_sozluk.txt ve customer_service_words.txt'den
        2+ kelimelik ifadeleri common_phrases'e ekle"""
        import os
        seen = set(p.lower() for p in self.common_phrases)

        # 1. features/ klasöründeki kısa sözlük
        path1 = os.path.join(os.path.dirname(__file__), "musteri_hizmetleri_sozluk.txt")
        # 2. data/ klasöründeki büyük telecom sözlüğü
        path2 = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "customer_service_words.txt")
