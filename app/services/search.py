...
# NOTE: CUSTOMER_SERVICE_WORDS is loaded lazily inside _local_search
# to avoid circular imports (ai.py <-> search.py)
        ...
        if LARGE_DICT_AVAILABLE and large_dictionary:
            try:
                ...
                if results:
                    ...
                    try:
                        from app.services.ai import CUSTOMER_SERVICE_WORDS as _cs
                    except Exception:
                        _cs = []
                    seen_cs = {s.text for s in suggestions}
                    if len(prefix_lower) >= 2 and _cs:
                        for phrase in _cs:
